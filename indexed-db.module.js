export const manifest = {
    ...
    permissions: [],
    actions: ["openDb", "addRecord", "getRecord", "getAllRecords", "removeRecord", "updateRecord", "getByIndex", "countRecords"],
};
...
export const getRecord = async (params) => await promisify(getStore(params).get(params.key));
export const getAllRecords = async (params) => await promisify(getStore(params).getAll());
export const removeRecord = async (params) => await promisify(getStore(params, 'readwrite').delete(params.key));
export const countRecords = async (params) => await promisify(getStore(params).count());
export const getByIndex = async (params) => {
    const { indexName, value, limit, direction = 'next' } = params;
    const index = getStore(params).index(indexName);
    if (limit) return await iterateCursor(index.openCursor(value ? IDBKeyRange.only(value) : null, direction), () => true, limit);
