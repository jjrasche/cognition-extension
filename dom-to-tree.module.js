...
const getIdAttributes = (el, tag, attrs) => el.id && (attrs.id = el.id);
const getClassAttributes = (el, tag, attrs) => el.className && (attrs.class = el.className);
...
export const test = async () => {
	...
	return [
		...
		// 	const doc = createTestDoc('<img src="https://example.com/test.jpg" alt="Test image">');
		// 	const actual = (await createTree(doc)).tree["img-1"];
		// 	const expected = { tag: "img", src: "https://example.com/test.jpg", alt: "Test image" };
		// 	return { actual, assert: deepEqual, expected };
		// }),
		// Hierarchy Tests
		await runUnitTest("Maintains parent-child relationships", async () => {
			const doc = createTestDoc('<div id="parent-div" class="container">Parent<p id="child-p">Child</p></div>');
			const actual = (await createTree(doc)).tree;
            const divObj = { tag: "div", id: "parent-div", class: "container", text: "Parent" };
            const pObj = { tag: "p", id:"child-p", text: "Child" };
			return { actual, assert: deepEqual, expected: { [divObj.id]: divObj, [pObj.id]: pObj } };
		}),
		...
		// 	debugger;
		// 	const actual = { totalElements: 40, hasDepth: 5,
