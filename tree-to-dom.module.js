...
const setupTextSelection = (el, id, elements) => (el.style.userSelect = 'text', ['mouseup', 'keyup'].forEach(evt => el.addEventListener(evt, (e) => handleSelectionEvent(e, id, elements)))); const setupSelectDoneListeners = (el, id, elements) => ['mouseup', 'keyup'].forEach(eventType => el.addEventListener(eventType, (e) => handleSelectionEvent(e, id, elements)));
const handleSelectionEvent = async (event, elementId, elements) => {
	let selection = window.getSelection();
	if (!selection || selection.rangeCount === 0) return;
	const text = selection.toString().trim();
	const element = elements[elementId];
	const handler = element.getAttribute('data-text-selection-handler');
	try {
		await runtime.call(handler, { ...createEventData(event, element), text, elementId, range: { startOffset: selection.getRangeAt(0).startOffset, endOffset: selection.getRangeAt(0).endOffset, startContainer: selection.getRangeAt(0).startContainer.textContent } });
	} catch (error) {
...
export const test = async () => {
	...
	results.push(await runUnitTest("Text Selection: mouseup calls textSelection handler with selection data", async () => {
		const divObj = {
			...
			data: { textSelectionHandler: "test.handleTextSelection" }, // Use data property instead
			text: "This is selectable text content for testing purposes."
		};
		const tree = { [divObj.id]: divObj };
		...
		window.getSelection = () => ({
			...
			rangeCount: 1, // Add missing rangeCount
			getRangeAt: () => ({
				startOffset: 5,
				endOffset: 18,
				startContainer: { textContent: "This is selectable text content for testing purposes." }
			})
		});
		...
		try {
			...
			// Simulate mouseup event with text selection
			element.dispatchEvent(new MouseEvent('mouseup', { bubbles: true }));

			const actual = {
				action: runtimeCalls[0]?.action,
				hasSelection: !!runtimeCalls[0]?.data?.selection,
				selectedText: runtimeCalls[0]?.data?.selection?.text,
				elementId: runtimeCalls[0]?.data?.target?.id
			};

			const expected = {
				action: "test.handleTextSelection",
				hasSelection: true,
				selectedText: "selected text",
				elementId: "selectable-content"
			};
