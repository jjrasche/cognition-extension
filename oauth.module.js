...
const waitForToken = (provider, timeout = 30000) => new Promise((resolve) => {
	...
	const check = () => {
		...
		if (token) {
			...
			console.error(` Timeout waiting for token for ${provider}`);
			resolve(null);
		} else {
...
const doRefresh = async (provider) => {
	...
	try {
		...
		console.error(` Refresh failed for ${provider}:`, error);
		await clearTokens({ provider });
		return null;
	}
};
...
const handleCallback = async (callbackUrl) => {
	...
	if (params.error) {
		console.error(` Callback error: ${params.error}`);
	...
	if (!provider) {
		console.error(' Invalid state - possible CSRF');
